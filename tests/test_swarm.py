...
def test_tool_argument_preprocessing():
    """Verify tool_dispatcher argument preprocessing handles flat parameters, alternate names, and points."""
    calls = []

    def fake_create_soil_material(name, model, params):
        calls.append(("create_soil_material", name, model, params))
        return "soil_mat_created"

    def fake_assign_material(object_name, material_name):
        calls.append(("assign_material", object_name, material_name))
        return "assigned"

    def fake_create_borehole(x, y, layers):
        calls.append(("create_borehole", x, y, layers))
        return "borehole_created"

    with patch.dict(
        "tool_dispatcher.TOOL_REGISTRY",
        {
            "create_soil_material": fake_create_soil_material,
            "assign_material": fake_assign_material,
            "create_borehole": fake_create_borehole,
        },
        clear=False,
    ):
        # Test create_soil_material with top-level params
        dispatch_tool_calls([
            {
                "name": "create_soil_material",
                "args": {
                    "name": "Clay_Layer",
                    "model": "Mohr-Coulomb",
                    "gammaUnsat": 16,
        ...
        # Test assign_material with alternate names
        dispatch_tool_calls([
            {
                "name": "assign_material",
                "args": {
                    "layer_name": "Soillayer_1",
                    "materials": "Clay_Layer",
                }
            }
        ])

        # Test create_borehole with a point list
        dispatch_tool_calls([
            {
    ...
    assert len(calls) == 3
    assert calls[0] == ("create_soil_material", "Clay_Layer", "Mohr-Coulomb", {"gammaUnsat": 16, "gammaSat": 19, "Eref": 20000})
    assert calls[1] == ("assign_material", "Soillayer_1", "Clay_Layer")
    assert calls[2] == ("create_borehole", 1.5, -2.5, [{"top": 0, "bottom": -5}])
